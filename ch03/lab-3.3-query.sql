...
SELECT Sname, Sno, Smajor FROM en.Student;
...
SELECT DISTINCT Sno FROM en.SC;

-- 例3.19 查询全体学生的姓名及其年龄
-- 方法1：使用EXTRACT函数计算年龄
SELECT Sname, EXTRACT(YEAR FROM age(current_date, Sbirthdate)) AS Sage 
FROM en.Student;

-- 方法2：使用date_part函数计算年龄
SELECT Sname, date_part('year', age(current_date, Sbirthdate)) AS Sage 
FROM en.Student;

-- 方法3：使用age函数并格式化输出
SELECT Sname, age(current_date, Sbirthdate) AS age_interval 
FROM en.Student;

-- 例3.20 查询考试成绩不及格的学生学号
SELECT DISTINCT Sno FROM en.SC WHERE Grade < 60;
...
SELECT Sname, Sbirthdate, Smajor FROM en.Student
...
-- 例3.42 查询选修了课程的学生人数
SELECT COUNT(DISTINCT Sno) FROM en.SC;

-- 例3.43 计算选修81001号课程的学生平均成绩
SELECT AVG(Grade) FROM en.SC
WHERE Cno = '81001';
...
SELECT SUM(Ccredit)
FROM en.SC, en.Course
WHERE Sno = '20180003' AND en.SC.Cno = en.Course.Cno;

-- 3.3.1.5 GROUP BY子句
-- 例3.46 求各个课程号及选修该课程的人数
SELECT Cno, COUNT(Sno)
FROM en.SC GROUP BY Cno;

-- 例3.47 查询2019年第2学期选修课程数超过10门的学生学号
SELECT Sno
FROM en.SC
WHERE Semester = '20192' GROUP BY Sno
HAVING COUNT(*) > 10;

-- 例3.48 查询平均成绩大于或等于90分的学生学号和平均成绩
SELECT Sno, AVG(Grade) FROM en.SC
GROUP BY Sno
HAVING AVG(Grade) >= 90;
...
SELECT Sno
FROM en.SC, en.Course
WHERE en.Course.Cname = '数据库系统概论' AND en.SC.Cno = en.Course.Cno
ORDER BY Grade DESC
LIMIT 10;
-- 取前10行数据为查询结果
...
SELECT en.Student.*, en.SC.*
FROM en.Student, en.SC
WHERE en.Student.Sno = en.SC.Sno;
-- 将Student与SC中同一学生的元组连接起来

-- 例3.52 查询每个学生的学号、姓名、性别、出生日期、主修专业及该学生选修课程的课程号与成绩
SELECT en.Student.Sno, Sname, Ssex, Sbirthdate, Smajor, Cno, Grade 
FROM en.Student, en.SC
WHERE en.Student.Sno = en.SC.Sno;

-- 例3.53 查询选修81002号课程且成绩在90分以上的所有学生的学号和姓名
SELECT en.Student.Sno, Sname
FROM en.Student, en.SC
WHERE en.Student.Sno = en.SC.Sno AND
      en.SC.Cno = '81002' AND en.SC.Grade > 90;
...
SELECT en.Student.Sno, Sname, Ssex, Sbirthdate, Smajor, Cno, Grade
FROM en.Student LEFT OUTER JOIN en.SC ON (en.Student.Sno = en.SC.Sno);
...
SELECT en.Student.Sno, Sname, Cname, Grade
FROM en.Student, en.SC, en.Course
WHERE en.Student.Sno = en.SC.Sno AND en.SC.Cno = en.Course.Cno;
...
SELECT Sname FROM en.Student WHERE Sno IN
(SELECT Sno FROM en.SC
WHERE Cno = '81003');
-- 外层查询或父查询内层查询或子查询
...
-- 3. 构造嵌套查询
SELECT Sno, Sname, Smajor FROM en.Student
WHERE Smajor = (SELECT Smajor FROM en.Student WHERE Sname = '刘晨');
